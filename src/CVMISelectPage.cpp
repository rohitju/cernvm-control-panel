...
downloadListURL("http://rbuilder.cern.ch/downloads/buildlist.py?plain=y&hypervisor=xen"),
availableImagesList("")
{
  // Filter group box 
  ...
  QLabel* hyperlabel=new QLabel(tr("Hypervisor:"));
  hyps =new QButtonGroup();
  xen=new QRadioButton("Xen");
  xen->setChecked(true);
  Q_ASSERT(connect(xen,SIGNAL(clicked(bool)),this,SLOT(xenChecked(bool))));
  vbox=new QRadioButton("VirtualBox");
  Q_ASSERT(connect(vbox,SIGNAL(clicked(bool)),this,SLOT(vboxChecked(bool))));
  vmw=new QRadioButton("VMware");
  Q_ASSERT(connect(vmw,SIGNAL(clicked(bool)),this,SLOT(vmwChecked(bool))));
  qemu=new QRadioButton("QEMU");
  Q_ASSERT(connect(qemu,SIGNAL(clicked(bool)),this,SLOT(qemuChecked(bool))));
 hyps->addButton(xen);
 hyps->addButton(vbox);
 hyps->addButton(vmw);
 hyps->addButton(qemu);
  ...
  filterLayout->addWidget(hyperlabel,0,2);
  filterLayout->addWidget(xen,1,2);
  filterLayout->addWidget(vbox,2,2);
  filterLayout->addWidget(vmw,3,2);
  filterLayout->addWidget(qemu,4,2);
...
void CVMISelectPage::xenChecked(bool checked)
{
    downloadListURL="http://rbuilder.cern.ch/downloads/buildlist.py?plain=y&hypervisor=xen";
    populateList();
}

void CVMISelectPage::vboxChecked(bool checked)
{
    downloadListURL="http://rbuilder.cern.ch/downloads/buildlist.py?plain=y&hypervisor=virtualbox";
    populateList();
}

void CVMISelectPage::vmwChecked(bool checked)
{
    downloadListURL="http://rbuilder.cern.ch/downloads/buildlist.py?plain=y&hypervisor=vmware";
    populateList();
}

void CVMISelectPage::qemuChecked(bool checked)
{
    downloadListURL="http://rbuilder.cern.ch/downloads/buildlist.py?plain=y&hypervisor=qemu";
    populateList();
}
